...
本项目为 [DiskNext Server](../Server/) 提供面向公众的前端服务，基于 pnpm + Vue + NuxtUI 构建。

提交前必须按照 pnpm lint -> build -> typecheck 进行检查。

## 空状态设计规范

页面数据为空时，采用**教育性模拟 UI 图示**风格，通过模拟真实界面元素直观展示用户应如何操作，而非简单的图标+文字。
