...
export function UploadPanel() {
  ...
  const summary = useMemo(() => {
    ...
    const totalProgress = total > 0 ? Math.round(tasks.reduce((sum, t) => sum + t.progress, 0) / total) : 0;
    return { total, complete, error, active, totalProgress };
  }, [tasks]);

  if (!panelOpen || tasks.length === 0) return null;

  const hasFinished = tasks.some((t) => t.status === "complete" || t.status === "cancelled");
    ...
    <div className="fixed bottom-4 right-4 z-50 w-96 rounded-lg border bg-popover shadow-xl">
      <div className="flex items-center gap-2 border-b px-3 py-2">
        <h3 className="flex-1 text-sm font-medium">
          {summary.active > 0 ? t("upload.uploading", { count: summary.active }) : t("upload.title")}
        </h3>
        <div className="flex items-center gap-0.5">
          {hasFinished && (
            <Tooltip>
              <TooltipTrigger render={<Button variant="ghost" size="icon-sm" onClick={clearCompleted} />}>
                <Trash2 className="size-3.5" />
              </TooltipTrigger>
              <TooltipContent>{t("upload.clearCompleted")}</TooltipContent>
            </Tooltip>
          )}
          <Button variant="ghost" size="icon-sm" onClick={() => setPanelMinimized(!panelMinimized)}>
      ...
      {summary.active > 0 && !panelMinimized && (
        <div className="px-3 pt-2">
          ...
          <div className="flex items-center justify-between mt-1 text-xs text-muted-foreground">
            ...
            {summary.error > 0 && <span className="text-destructive">{t("upload.errorCount", { count: summary.error })}</span>}
          </div>
        </div>
      )}
      ...
      {!panelMinimized && (
        <ScrollArea className="max-h-72">
          <div className="divide-y">
            {tasks.map((task) => (
              <UploadTaskItem key={task.id} task={task} onCancel={cancelTask} onRetry={retryTask} onRemove={removeTask} />
            ))}
          </div>
        </ScrollArea>
      )}
